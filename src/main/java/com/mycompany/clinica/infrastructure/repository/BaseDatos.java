package com.mycompany.clinica.infrastructure.repository;

import com.mycompany.clinica.infrastructure.execption.TecnicoException;
...
public class BaseDatos {
    ...
    public int insertSignosVitales(VitalSigns signosVitales, int idConsulta) {
    ...
    // capturar, declarar
    public int insertConsulta(Consultation consulta, int idPaciente) {
    ...
    public List<Consultation> obtenerConsultas(int idPaciente) {
        ...
        try (PreparedStatement pst = ConnectionDB.getInstance().prepareStatement("SELECT * FROM consulta WHERE id_paciente = ?")) {
            ...
            while (rs.next()) {
                ...
                VitalSigns signosVitales = rs.get
                Consultation consulta = new Consultation(rs.getString(4), localDate, rs.getString(5), rs.getString(6), 
                        rs.getString(7), );
    ...
    public List<VitalSigns> obtenerSignosVitales(int idConsulta) {
        ...
        List<VitalSigns> signos = new ArrayList<>();
        ...
        if (paciente != null) {
            ...
            try (PreparedStatement psConsulta = ConnectionDB.getInstance().prepareStatement(sqlConsulta)) {
                ...
                while (rsConsulta.next()) {
                    ...
                    Consultation consulta = new Consultation(diagnostico, receta, indicaciones, fechaConsulta);
                    consultas.add(consulta);
                }
            } catch (SQLException e) {
